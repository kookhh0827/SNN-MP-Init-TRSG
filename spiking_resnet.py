...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, init_tau=2.0, init_thr=2.0):
    ...
    def forward(self, x):
        ...
        return self.sn2(self.conv2(self.sn1(self.conv1(x))) + self.downsample(x))
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        return self.sn3(self.conv3(self.sn2(self.conv2(self.sn1(self.conv1(x))))) + self.downsample(x))
...
class SpikingResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 ...
                 norm_layer=None, T=4, init_tau=2.0, init_thr=2.0):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):
        ...
        layers.append(block(self.inplanes, planes, stride, downsample, self.groups,
                            self.base_width, previous_dilation, norm_layer, self.init_tau, self.init_thr))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups=self.groups,
                                base_width=self.base_width, dilation=self.dilation,
                                norm_layer=norm_layer, init_tau=self.init_tau, init_thr=self.init_thr))
